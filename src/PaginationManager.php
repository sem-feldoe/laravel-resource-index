...
use AaronFrancis\FastPaginate\FastPaginate;
...
final class PaginationManager
{
    public function paginate(
        ...
        if (class_exists(\Hammerstone\FastPaginate\Hammerstone\FastPaginate::class) ||
            class_exists(FastPaginate::class)) {
    ...
    public function applyLimitOffset(EloquentBuilder|ScoutBuilder|null $query, ?int $limit, ?int $offset): Collection
    {
        if (! is_null($limit)) {
            $query->take($limit);
        }
